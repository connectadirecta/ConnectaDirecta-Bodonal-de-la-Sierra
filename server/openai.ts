...
function buildSystemPrompt(user: User) {
  ...
  return [
    ...
    "INFORMACIÓN BIOGRÁFICA CRÍTICA PARA REMINISCENCIA:",
    "ÚSALA ACTIVAMENTE en cada conversación para validar, conectar y profundizar en los recuerdos de la persona.",
    `<BIOGRAPHICAL_INFO>
        lugar_nacimiento: ${user.birthPlace ?? "pregunta sobre esto"}
        hogar_infancia: ${user.childhoodHome ?? "pregunta sobre esto"}
        recuerdos_infancia: ${user.childhoodMemories ?? "pregunta sobre esto"}
        historia_familiar: ${user.familyBackground ?? "pregunta sobre esto"}
        hermanos: ${user.siblings ?? "pregunta sobre esto"}
        padres: ${user.parents ?? "pregunta sobre esto"}
        eventos_significativos: ${user.significantLife ?? "pregunta sobre esto"}
        profesión: ${user.profession ?? "pregunta sobre esto"}
        pasatiempos: ${user.hobbies ?? "pregunta sobre esto"}
        recuerdos_favoritos: ${user.favoriteMemories ?? "pregunta sobre esto"}
      </BIOGRAPHICAL_INFO>`,
    "",
    "IMPORTANTE: Cuando la persona comparte un recuerdo (infancia, familia, trabajo), SIEMPRE:",
    "1. Valida emocionalmente el recuerdo",
    "2. Haz UNA pregunta específica para profundizar",
    "3. Conecta con información que ya conoces de ellos",
...
function ruleBasedReply(userMessage: string, user: User) {
  ...
  // Orientación temporal - pero SOLO si no hay contexto biográfico previo en el mensaje
  if (/(qué hora|que hora|hora|fecha|qué día|que día)/i.test(msg)) {
    // Si el mensaje tiene contexto biográfico/emocional, no usar regla rápida
    const hasBiographicalContext = /(pequeño|infancia|hermano|padre|madre|familia|recuerdo|cuando era|trabajaba|vivía)/i.test(msg);
    if (!hasBiographicalContext) {
      return `${user.firstName}, ahora son las ${toTimeES()}. Hoy es ${toDateES()}. ¿Cómo te va hasta ahora?`;
    }
    // Si hay contexto biográfico, dejar que el modelo con memoria responda
    return null;
  }
...
async function maybeUpdateRollingSummary(
  ...
  try {
    ...
    // Heurística más agresiva: actualizar más frecuentemente para capturar información
    const deltaLen = userText.length + assistantText.length;
    const should = deltaLen > 300 || Math.random() < 0.5; // Más frecuente
    if (!should) return;
    ...
    const msgs: ChatCompletionMessageParam[] = [
      {
        role: "system",
        content:
          "Eres un asistente que mantiene un RESUMEN BREVE en español (4–6 frases). PRIORIZA capturar: recuerdos personales, información biográfica, familia, gustos, hobbies, experiencias de vida. Mantén hechos clave y relaciones mencionadas. No repitas. Actualiza el resumen previo añadiendo nueva información relevante.",
      },
      {
        role: "user",
        content: `Resumen previo:\n${prev || "(vacío)"}\n\nNueva interacción:\nUSUARIO: ${userText}\nASISTENTE: ${assistantText}\n\nDevuelve SOLO el resumen actualizado, incluyendo cualquier recuerdo o información personal mencionada.`,
      },
    ];
...
async function extractAndUpsertMemories(
  ...
  try {
    ...
    const sys = `Eres un extractor de MEMORIAS BIOGRÁFICAS en español. Tu trabajo es CRÍTICO para la terapia de reminiscencia.

REGLAS ESTRICTAS:
1. Devuelve SOLO JSON válido con un array "memories"
2. CAPTURA TODO recuerdo biográfico, familiar, de infancia, profesional o personal
3. PRIORIDAD MÁXIMA: memorias de infancia, familia, juventud, trabajo, hobbies
4. NO ignores ninguna mención de hermanos, padres, lugares de nacimiento, experiencias de vida
5. type ∈ {PREFERENCE, ROUTINE, CONTACT, FACT, GOAL, HEALTH_NOTE}
6. importance: 5 para memorias de infancia/familia, 4 para trabajo/hobbies, 3 para gustos generales
7. NO incluyas datos clínicos ni diagnósticos

EJEMPLOS CRÍTICOS - DEBES capturar TODO como estos:
- "De pequeño me encantaba pasarme horas jugando con mis hermanos en el campo" → FACT, importance: 5, content: "Jugaba con sus hermanos en el campo durante la infancia"
- "Tengo tres hermanos" → CONTACT, importance: 5
- "Mi padre era agricultor" → FACT, importance: 5
- "Trabajé 30 años como carpintero" → FACT, importance: 4
- "Me gusta la sopa" → PREFERENCE, importance: 3
- "Mi nieto Lucas tiene 8 años" → CONTACT, importance: 4

SI el usuario menciona CUALQUIER cosa sobre su pasado, familia, infancia o vida, DEBES crear una memoria.
NO devuelvas array vacío si hay información biográfica en el mensaje.`;
    ...
    const comp = await withRetry(() =>
      openai!.chat.completions.create({
        model: DEFAULT_MODEL,
        temperature: 0.05, // Más determinista para extracción consistente
        max_tokens: 600, // Más tokens para capturar múltiples memorias
        messages: msgs,
    ...
    console.log("[Memory] Extraction result:", raw);
    ...
    if (items.length) {
      console.log("[Memory] Upserting memories:", items);
